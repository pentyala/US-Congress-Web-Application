...
if($op=="legislators"){
	...
	$json=json_decode(file_get_contents("http://congress.api.sunlightfoundation.com/legislators?per_page=1&apikey=1b18e2b4ba5b4865b27980b39d7f47b9&chamber=".$category),true);
	$count=$json['count'];
	$pg_no=0;
	$res=array();
	for ($i=0; $i < $count ; $i++) { 
		if($i%50==0)
			$json=json_decode(file_get_contents("http://congress.api.sunlightfoundation.com/legislators?per_page=50&page=".(++$pg_no)."apikey=1b18e2b4ba5b4865b27980b39d7f47b9&order=state_name__asc&chamber=".$category),true);
		...
		if(isset($json['results'][$i%50]['district']))
			$res['results'][$i]['district']=$json['results'][$i%50]['district'];
		else
			$res['results'][$i]['district']=null;
...
elseif ($op=="committees") {
	if(!isset($_GET['category']))
		die("Wrong inputs");
	$category=$_GET['category'];
	$json=json_decode(file_get_contents("http://congress.api.sunlightfoundation.com/committees?per_page=50&order=committee_id__asc&apikey=1b18e2b4ba5b4865b27980b39d7f47b9&chamber=".$category),true);
	$count=$json['count'];
	$pg_no=0;
	$res=array();
	for ($i=0; $i < $count ; $i++) { 
		if($i%50==0)
			$json=json_decode(file_get_contents("http://congress.api.sunlightfoundation.com/committees?per_page=50&order=committee_id__asc&page=".(++$pg_no)."apikey=1b18e2b4ba5b4865b27980b39d7f47b9&chamber=".$category),true);
		$res['results'][$i]['chamber']=$json['results'][$i%50]['chamber'];
...
elseif ($op=="bills") {
	...
	if($_GET['category']=="active")
		echo file_get_contents("http://congress.api.sunlightfoundation.com/bills?per_page=50&history.active=true&last_version.urls.pdf__exists=true&order=introduced_on__desc&apikey=1b18e2b4ba5b4865b27980b39d7f47b9");
	else
		echo file_get_contents("http://congress.api.sunlightfoundation.com/bills?per_page=50&last_version.urls.pdf__exists=true&history.active=false&order=introduced_on__desc&apikey=1b18e2b4ba5b4865b27980b39d7f47b9");
}
elseif ($op=="legis_comm") {
	if(!isset($_GET['bio_id']))
		die("error at line 135");
	$bio_id=$_GET['bio_id'];
	echo file_get_contents("http://congress.api.sunlightfoundation.com/committees?per_page=5&apikey=1b18e2b4ba5b4865b27980b39d7f47b9&member_ids=".$bio_id);
}
elseif ($op=="legis_bills") {
	if(!isset($_GET['bio_id']))
		die("error at line 141");
	$bio_id=$_GET['bio_id'];
	$json = json_decode(file_get_contents("http://congress.api.sunlightfoundation.com/bills?per_page=5&apikey=1b18e2b4ba5b4865b27980b39d7f47b9&sponsor_id=".$bio_id),true);
